...
func (i *Interp) buildImportlibResources() *object.Module {
	...
	// Traversable stub class
	traversableCls := &object.Class{Name: "Traversable", Dict: object.NewDict()}
	makeTraversable := func(pkg string) *object.Instance {
		inst := &object.Instance{Class: traversableCls, Dict: object.NewDict()}
		inst.Dict.SetStr("_pkg", &object.Str{V: pkg})
		inst.Dict.SetStr("name", &object.Str{V: pkg})
	...
	m.Dict.SetStr("files", &object.BuiltinFunc{Name: "files",
		Call: func(_ any, a []object.Object, _ *object.Dict) (object.Object, error) {
			...
			return makeTraversable(pkg), nil
		}})

	// as_file(path) context manager → yields path unchanged
	asFileCls := &object.Class{Name: "as_file_ctx", Dict: object.NewDict()}
	m.Dict.SetStr("as_file", &object.BuiltinFunc{Name: "as_file",
		Call: func(_ any, a []object.Object, _ *object.Dict) (object.Object, error) {
			...
			inst := &object.Instance{Class: asFileCls, Dict: object.NewDict()}
	...
	// Stub functions
	for _, fnName := range []string{"contents", "is_resource", "open_binary", "open_text", "read_binary", "read_text"} {
		n := fnName
		m.Dict.SetStr(n, &object.BuiltinFunc{Name: n,
			Call: func(_ any, _ []object.Object, _ *object.Dict) (object.Object, error) {
				return object.None, nil
			}})
	}

	// path context manager stub
	pathCtxCls := &object.Class{Name: "path_ctx", Dict: object.NewDict()}
	m.Dict.SetStr("path", &object.BuiltinFunc{Name: "path",
		Call: func(_ any, a []object.Object, _ *object.Dict) (object.Object, error) {
			a = mpArgs(a)
			inst := &object.Instance{Class: pathCtxCls, Dict: object.NewDict()}
			inst.Dict.SetStr("__enter__", &object.BuiltinFunc{Name: "__enter__",
				Call: func(_ any, _ []object.Object, _ *object.Dict) (object.Object, error) {
					return object.None, nil
				}})
	...
	// Type aliases (non-callable stubs)
...
func (i *Interp) buildImportlibMetadata() *object.Module {
	...
	m.Dict.SetStr("version", &object.BuiltinFunc{Name: "version",
		Call: func(_ any, a []object.Object, _ *object.Dict) (object.Object, error) {
			...
			// Try reading from real disk package metadata.
			if v := findInstalledVersion(name); v != "" {
	...
	// Stub classes
	for _, name := range []string{"Distribution", "PathDistribution", "EntryPoint", "EntryPoints", "PackagePath", "PackageMetadata"} {
		n := name
		cls := &object.Class{Name: n, Dict: object.NewDict()}
		m.Dict.SetStr(n, cls)
	}
...
// findInstalledVersion tries to read METADATA/PKG-INFO from site-packages for name.
func findInstalledVersion(name string) string {
	// Normalise package name: hyphens/underscores are equivalent.
	...
	for _, base := range candidates {
		...
		for _, e := range entries {
			...
			dir := e.Name()
			// dist-info directories: name-version.dist-info
			if !strings.HasSuffix(dir, ".dist-info") {
				continue
			}
			distName := strings.TrimSuffix(dir, ".dist-info")
